...
export default async function ClientDashboardPage() {
  ...
  const metricCards = [
    { label: "Volumen semanal", value: Math.round(weekVolume).toLocaleString("es-AR"), unit: "kg", Icon: TrendingUp },
    { label: "Entrenamiento", value: weekSessions.toString(), unit: "sesiones", Icon: Dumbbell },
    { label: "Peso actual", value: currentWeight ? currentWeight.toFixed(1) : "—", unit: "kg", Icon: Scale },
    { label: "Racha", value: streak.toString(), unit: "días", Icon: Flame },
  ];
  ...
  return (
    <div className="space-y-6 pb-8 text-white">
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="relative min-h-[390px] overflow-hidden rounded-[30px] border border-primary/15 bg-gradient-to-br from-[#111316] via-[#0A0D10] to-[#08090B] p-7 shadow-[0_24px_80px_rgba(0,0,0,0.32)] sm:p-9"><div className="absolute -right-24 -top-28 h-80 w-80 rounded-full bg-primary/10 blur-3xl" /><div className="absolute -bottom-32 left-1/3 h-72 w-72 rounded-full bg-primary/[0.05] blur-3xl" /><div className="absolute inset-0 bg-[radial-gradient(circle_at_74%_34%,rgba(214,255,42,0.10),transparent_33%),linear-gradient(135deg,rgba(255,255,255,0.025),transparent_50%)]" /><div className="relative flex h-full flex-col"><div className="flex items-center justify-between gap-4"><span className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-3.5 py-1.5 text-[10px] font-black uppercase tracking-[0.20em] text-primary"><span className="h-1.5 w-1.5 rounded-full bg-primary animate-pulse" /> {todayLog ? "Entrenamiento completado" : "Entrenamiento de hoy"}</span><span className="flex items-center gap-1.5 text-xs font-bold text-zinc-500"><Clock3 size={14} /> {todayLog?.durationMin || nextWorkout?.estimatedMin || 60} min</span></div><div className="mt-6 max-w-2xl"><p className="text-xs font-bold uppercase tracking-[0.2em] text-zinc-600">Tu sesión</p><h2 className="mt-2 text-4xl font-display font-black leading-[1.05] tracking-tight sm:text-5xl">{todayLog?.workout?.name || nextWorkout?.name || "Tu coach está preparando tu plan"}</h2><p className="mt-3 max-w-lg text-sm leading-6 text-zinc-500">{todayLog ? "Sesión registrada. Revisá el detalle y mantené el ritmo." : nextWorkout ? `${nextWorkout.exercises.length} ejercicios seleccionados para tu plan.` : "Cuando tu coach asigne un programa, aparecerá acá."}</p></div>{nextWorkout && !todayLog && <div className="mt-6 max-w-xl space-y-2">{nextWorkout.exercises.slice(0, 3).map((exercise, index) => <div key={exercise.id} className="flex items-center gap-3 rounded-2xl border border-white/[0.05] bg-black/30 px-3.5 py-2.5 backdrop-blur-sm"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-primary/10 text-[11px] font-black text-primary">{index + 1}</span><span className="min-w-0 flex-1 truncate text-sm font-semibold text-white">{exercise.exercise.name}</span><span className="text-xs font-bold tabular-nums text-zinc-500">{exercise.sets} × {exercise.reps}</span></div>)}</div>}<div className="mt-auto pt-7"><Link href={todayLog ? `/client/workout/${todayLog.workoutId}` : nextWorkout ? `/client/workout/${nextWorkout.id}` : "/client/workout"} className="block"><Button variant="accent" className="h-14 w-full rounded-2xl text-base font-black tracking-wide shadow-[0_10px_42px_rgba(214,255,42,0.18)]">{todayLog ? "VER ENTRENAMIENTO" : "COMENZAR ENTRENAMIENTO"}<ArrowRight size={18} className="ml-2" /></Button></Link></div></div></div>
        <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-1"><div className="rounded-[26px] border border-white/[0.06] bg-white/[0.025] p-5 shadow-[0_18px_60px_rgba(0,0,0,0.22)]"><div className="flex items-center justify-between"><div className="flex items-center gap-2 text-sm font-black"><Goal size={18} className="text-primary" /> Tu objetivo</div><Link href="/client/progress" className="text-[10px] font-bold uppercase tracking-wider text-primary">Ver detalles</Link></div><div className="mt-5 flex items-center gap-4"><div className="relative flex h-20 w-20 shrink-0 items-center justify-center rounded-full border-[7px] border-primary/20"><div className="absolute inset-0 rounded-full border-[7px] border-primary border-r-transparent -rotate-45" /><span className="text-xl font-black">{adherence}%</span></div><div><p className="text-lg font-black text-white">{currentWeight ? formatKg(currentWeight) : "Sin peso"}</p><p className="mt-1 text-xs text-zinc-500">adherencia del plan</p></div></div></div><div className="rounded-[26px] border border-primary/20 bg-primary/[0.035] p-5 shadow-[0_18px_60px_rgba(0,0,0,0.22)]"><div className="flex items-center justify-between"><div className="flex items-center gap-2 text-sm font-black"><Dumbbell size={18} className="text-primary" /> Próximo entrenamiento</div><span className="text-primary"><Play size={16} fill="currentColor" /></span></div><p className="mt-5 text-lg font-black">{todayLog ? "Siguiente sesión" : nextWorkout?.name || "Por definir"}</p><p className="mt-1 text-xs text-zinc-500">{todayLog ? "Tu próxima sesión aparece en tu programa." : nextWorkout ? `${nextWorkout.estimatedMin || 60} min · ${nextWorkout.exercises.length} ejercicios` : "Tu coach todavía no asignó un programa."}</p><Link href={nextWorkout ? `/client/workout/${nextWorkout.id}` : "/client/workout"} className="mt-5 flex h-10 items-center justify-center rounded-xl bg-primary text-xs font-black text-black transition hover:brightness-105">Ver entrenamiento <ArrowRight size={14} className="ml-1" /></Link></div></div>
      </section>

      <section className="grid grid-cols-2 overflow-hidden rounded-[26px] border border-white/[0.06] bg-white/[0.025] lg:grid-cols-4">{metricCards.map(({ label, value, unit, Icon }, index) => <div key={label} className={`p-5 sm:p-6 ${index > 0 ? "border-l border-white/[0.06]" : ""} ${index > 1 ? "border-t border-white/[0.06] lg:border-t-0" : ""}`}><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.16em] text-zinc-600"><Icon size={13} />{label}</div><p className="mt-2 text-3xl font-display font-black tabular-nums">{value} <span className="text-xs font-bold text-zinc-600">{unit}</span></p><MiniSparkline active={weekSessions > 0} /></div>)}</section>

      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]"><div className="rounded-[26px] border border-white/[0.06] bg-white/[0.025] p-5 sm:p-6"><div className="mb-5 flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-zinc-600">Esta semana</p><h3 className="mt-1 text-xl font-display font-black">Tus entrenamientos</h3></div><Link href="/client/workout" className="text-xs font-bold text-primary">Ver todos</Link></div><div className="grid gap-3 md:grid-cols-2 2xl:grid-cols-4">{workoutCards.map((workout, index) => { const done = recentLogs.some((log) => log.workoutId === workout.id); return <Link key={workout.id} href={workout.id.startsWith("empty-") ? "/client/workout" : `/client/workout/${workout.id}`} className="group overflow-hidden rounded-2xl border border-white/[0.06] bg-black/20 transition hover:-translate-y-0.5 hover:border-primary/25"><div className="flex aspect-[1.65/1] items-end bg-gradient-to-br from-[#1A1F21] via-[#0F1417] to-[#090B0D] p-3"><span className="rounded-lg bg-primary px-2 py-1 text-[9px] font-black uppercase tracking-wider text-black">{done ? "Completado" : index === 0 ? "Hoy" : "Plan"}</span></div><div className="p-3.5"><div className="flex items-start justify-between gap-2"><p className="min-h-10 flex-1 text-sm font-black leading-5 text-white">{workout.name}</p><ArrowRight size={15} className="mt-0.5 text-zinc-700 transition group-hover:translate-x-0.5 group-hover:text-primary" /></div><p className="mt-1 text-xs text-zinc-600">{workout.estimatedMin || 45} min · {workout.exercises?.length || 0} ejercicios</p></div></Link>; })}</div></div><div className="rounded-[26px] border border-white/[0.06] bg-white/[0.025] p-5 sm:p-6"><div className="flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-zinc-600">Progreso</p><h3 className="mt-1 text-xl font-display font-black">Tu semana</h3></div><TrendingUp size={19} className="text-primary" /></div><div className="mt-6 space-y-4">{weekDays.map((day) => <div key={day.label} className="flex items-center gap-3"><span className={`w-5 text-center text-[10px] font-black ${day.isToday ? "text-primary" : "text-zinc-600"}`}>{day.label}</span><div className="h-2 flex-1 overflow-hidden rounded-full bg-white/[0.05]"><div className="h-full rounded-full bg-primary transition-all" style={{ width: `${Math.min(100, day.count * 100)}%` }} /></div><span className="w-4 text-right text-[10px] font-bold text-zinc-600">{day.count}</span></div>)}</div><div className="mt-7 grid grid-cols-2 gap-3"><div className="rounded-2xl border border-white/[0.05] bg-black/20 p-3"><p className="text-[10px] uppercase tracking-wider text-zinc-600">Adherencia</p><p className="mt-1 text-xl font-black text-primary">{adherence}%</p></div><div className="rounded-2xl border border-white/[0.05] bg-black/20 p-3"><p className="text-[10px] uppercase tracking-wider text-zinc-600">Racha</p><p className="mt-1 text-xl font-black">{streak} d</p></div></div></div></section>

      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]"><div className="rounded-[26px] border border-white/[0.06] bg-white/[0.025] p-5 sm:p-6"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><Utensils size={18} className="text-primary" /><div><p className="text-xs font-black uppercase tracking-[0.18em] text-zinc-600">Nutrición</p><h3 className="mt-1 text-xl font-display font-black">Tu plan de hoy</h3></div></div><Link href="/client/nutrition" className="text-xs font-bold text-primary">Ver plan completo</Link></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{["Desayuno", "Almuerzo", "Merienda", "Cena"].map((meal) => <Link key={meal} href="/client/nutrition" className="rounded-2xl border border-white/[0.05] bg-black/20 p-4 transition hover:border-primary/20"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary/10 text-primary"><Utensils size={18} /></div><p className="mt-3 text-sm font-black">{meal}</p><p className="mt-1 text-xs text-zinc-600">Abrir tu plan nutricional</p></Link>)}</div></div><div className="rounded-[26px] border border-white/[0.06] bg-white/[0.025] p-5 sm:p-6"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><Trophy size={18} className="text-primary" /><h3 className="text-xl font-display font-black">Logros</h3></div><Link href="/client/achievements" className="text-xs font-bold text-primary">Ver todos</Link></div><div className="mt-5 space-y-3">{[streak > 0 ? "Constancia" : "Primer día", weekSessions > 0 ? "Disciplina" : "Listo para empezar", adherence >= 75 ? "Progreso" : "Mantener ritmo"].map((label, i) => <div key={label} className="flex items-center gap-3 rounded-2xl border border-white/[0.05] bg-black/20 p-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/10 text-primary"><Sparkles size={17} /></span><div className="min-w-0 flex-1"><p className="text-sm font-black">{label}</p><p className="text-[10px] text-zinc-600">Actividad de tu panel</p></div><span className="text-[10px] font-black text-primary">+{(i + 1) * 50} XP</span></div>)}</div></div></section>
