...
export async function GET(req:Request){
  ...
  try{
    ...
    if(type==="progress"){
      ...
      if(checkin){ allowed=checkin.userId===session.id; if(!allowed&&checkin.clientId) allowed=Boolean(await prisma.client.findFirst({where:{id:checkin.clientId,OR:[{userId:session.id},{trainerId:session.id}]},select:{id:true}})); }
  ...
  }catch(error){ console.error("[UPLOAD-SERVE]",error); return NextResponse.json({error:"Archivo no encontrado o no disponible"},{status:404}); }
}
