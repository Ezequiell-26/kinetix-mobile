...
export async function POST(req: Request) {
  ...
  try {
    ...
    const requestedClientId = typeof form.get("clientId") === "string" ? String(form.get("clientId")).trim() : "";
    let clientId: string | null = null;
    if (requestedClientId) {
      if (session.role !== "TRAINER") return NextResponse.json({ error: "Solo un trainer puede cargar archivos para un cliente" }, { status: 403 });
      if (!(await assertTrainerOwnsClient(session.id, requestedClientId))) return NextResponse.json({ error: "Cliente no encontrado" }, { status: 404 });
    ...
    await prisma.privateAsset.create({ data: { userId: session.id, clientId, type: rawType, filename, contentType: file.type, sizeBytes: file.size } });
    ...
    if (rawType === "progress") {
      await prisma.progressPhoto.create({
        data: {
          ...
          clientId,
          url,
          note: typeof form.get("note") === "string" ? String(form.get("note")).slice(0, 500) : null,
          isPrivate: true,
        },
      });
    }
