...
export async function POST(request: NextRequest) {
  ...
  if (!supabase) return NextResponse.json({ error: "Servicio push no configurado" }, { status: 503 });
  ...
  try {
    ...
    if (!subscriptions || subscriptions.length === 0) return NextResponse.json({ success: true, sent: 0, failed: 0, total: 0 });

    const vapidPublicKey = process.env.VAPID_PUBLIC_KEY;
    const vapidPrivateKey = process.env.VAPID_PRIVATE_KEY;
    if (!vapidPublicKey || !vapidPrivateKey) return NextResponse.json({ error: "Push no configurado" }, { status: 503 });
    ...
    const results = await Promise.allSettled(
      subscriptions.map(async (sub: { endpoint: string; p256dh: string; auth: string }) => {
        try {
          ...
          if (statusCode === 404 || statusCode === 410) await supabase.from("push_subscriptions").delete().eq("endpoint", sub.endpoint);
