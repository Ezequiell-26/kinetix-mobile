...
declare global {
  interface Window {
    kinetixfitt?: { isElectron?: boolean };
    ezequiel?: { isElectron?: boolean }; // alias histórico
  }
}
...
export function PwaInstallDesktop(){
  ...
  useEffect(()=>{
    if(window.kinetixfitt?.isElectron || window.ezequiel?.isElectron) setIsElectron(true);
      ...
      <Card className="border-primary/20 bg-gradient-to-br from-primary/5 via-zinc-900 to-zinc-900">
        <CardContent className="pt-4 space-y-3">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3 space-y-2">
            ...
            <div className="flex gap-2 items-center text-xs text-emerald-400"><Check size={14}/> ¡Listo! Ícono KINETIXFITT en tu inicio, abre como app nativa</div>
          </div>
      ...
      <CardContent className="pt-4 space-y-3">
        ...
        {deferred ? (
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3 text-xs space-y-1">
            ...
            <p className="text-zinc-500">Chrome/Edge → menú <span className="font-bold text-white">⋮ → Instalar KINETIXFITT</span> o ícono <Download size={12} className="inline"/> en la barra de direcciones.</p>
          </div>
        )}
