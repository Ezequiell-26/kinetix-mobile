...
export function ExportCenter({ type="client" }:{ type?: "client"|"trainer"}){
  ...
  async function exportCsv(){
    setLoading("csv");setError(null);try{const {logs}=await load();const rows=[["fecha","entrenamiento","ejercicio","peso","reps","rir","duracion_min","comentario"]];for(const log of logs){for(const set of (log.sets||[])){rows.push([new Date(log.date).toLocaleDateString("es-AR"),log.workoutName||"Entrenamiento",set.exerciseName,set.weight??"",set.reps??"",set.rir??"",log.durationMin??"",log.comment||""]);}}download("kinetixfitt_historial.csv",rows.map(r=>r.map(csvCell).join(",")).join("\n"),"text/csv;charset=utf-8");}catch(e){setError(e instanceof Error?e.message:"No se pudo exportar.");}finally{setLoading(null);}}
