 ...
 *
 * La usan `POST /api/uploads` y los tests de seguridad. Nada de listas
 * duplicadas en cada ruta: si un tipo/extensión cambia, cambia ACÁ.
 */

// Tipos de upload permitidos (directorios bajo uploads/).
...
export function isUploadType(v: unknown): v is UploadType {
  return (
    typeof v === "string" &&
    (UPLOAD_TYPES as readonly string[]).includes(v)
  );
}
