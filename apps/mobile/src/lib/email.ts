...
export function getAppUrl(): string {
  const raw =
    ...
    process.env.WEB_URL ||
    process.env.APP_URL ||
    (process.env.NODE_ENV === "production" ? "https://kinetixfitt.com" : "http://localhost:3001");
  return raw.replace(/\/$/, "");
}
...
export function getEmailFrom(): string {
  ...
  return (
    ...
    (process.env.SMTP_USER ? `KINETIXFITT <${process.env.SMTP_USER}>` : "KINETIXFITT <noreply@kinetixfitt.com>")
  );
}
...
// ---------------------------------------------------------------------------
// Template premium KINETIXFITT — dark + lime (#D6FF2A / #09090B)
// ---------------------------------------------------------------------------
...
export function buildPasswordResetHtml(opts: { resetUrl: string; email: string }): string {
  ...
  const preheader = "Restablece tu contraseña en KINETIXFITT — enlace válido por 60 minutos.";
  return `<!DOCTYPE html>
...
<title>Restablece tu contraseña — KINETIXFITT</title>
</head>
    ...
    <tr>
      <td align="center">
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="width:100%;max-width:600px;background-color:#18181B;border-radius:16px;overflow:hidden;border:1px solid #27272A;">
          <!-- Header brand -->
          <tr>
            <td style="background-color:#09090B;padding:28px 40px;text-align:center;border-bottom:3px solid #D6FF2A;">
              <div style="font-family:Inter,Helvetica,Arial,sans-serif;font-weight:900;font-size:28px;letter-spacing:-0.04em;color:#ffffff;line-height:1;">KINETIX<span style="color:#D6FF2A;">FITT</span></div>
          ...
          <tr>
            <td style="padding:36px 40px 32px;">
              ...
              <p style="margin:0 0 28px;font-family:Inter,Helvetica,Arial,sans-serif;font-size:14px;line-height:1.7;color:#A1A1AA;">
                Haz clic en el botón para crear una nueva contraseña. Este enlace expira en <span style="color:#D6FF2A;font-weight:700;">60 minutos</span> y solo puede usarse una vez.
              </p>
              <!-- CTA -->
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td align="center" style="padding:4px 0 24px;">
                    <!--[if mso]>
                    <v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" href="${escapeAttr(resetUrl)}" style="height:48px;v-text-anchor:middle;width:320px;" arcsize="50%" fillcolor="#D6FF2A" stroke="f">
                    <center style="color:#09090B;font-family:Arial,sans-serif;font-size:14px;font-weight:800;letter-spacing:0.06em;">RESTABLECER CONTRASEÑA →</center>
                    </v:roundrect>
                    <![endif]-->
                    <!--[if !mso]><!-->
                    <a href="${escapeAttr(resetUrl)}" target="_blank" rel="noopener noreferrer" style="display:inline-block;background-color:#D6FF2A;color:#09090B;font-family:Inter,Helvetica,Arial,sans-serif;font-size:13px;font-weight:800;letter-spacing:0.08em;text-decoration:none;padding:15px 32px;border-radius:999px;line-height:1;text-transform:uppercase;">Restablecer contraseña →</a>
                    <!--<![endif]-->
                  </td>
                </tr>
              </table>
              ...
              <p style="margin:0 0 24px;word-break:break-all;text-align:center;">
                <a href="${escapeAttr(resetUrl)}" target="_blank" rel="noopener noreferrer" style="font-family:Inter,Helvetica,Arial,sans-serif;font-size:12px;color:#D6FF2A;text-decoration:underline;word-break:break-all;">${escapeHtml(resetUrl)}</a>
              </p>
              <!-- Security note -->
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#09090B;border:1px solid #27272A;border-radius:10px;">
                <tr>
              ...
              <p style="margin:16px 0 0;font-family:Inter,Helvetica,Arial,sans-serif;font-size:11px;line-height:1.6;color:#52525B;text-align:center;">
                Por seguridad, nunca compartas este enlace. El equipo de KINETIXFITT jamás te pedirá tu contraseña por email.
              </p>
            </td>
          </tr>
          <!-- Footer -->
          <tr>
            <td style="background-color:#09090B;padding:22px 40px;border-top:1px solid #27272A;text-align:center;">
              <p style="margin:0 0 6px;font-family:Inter,Helvetica,Arial,sans-serif;font-size:11px;line-height:1.6;color:#52525B;">
                © ${new Date().getFullYear()} KINETIXFITT. Todos los derechos reservados.
              </p>
              ...
              <p style="margin:10px 0 0;font-family:Inter,Helvetica,Arial,sans-serif;font-size:10px;color:#3F3F46;line-height:1.5;">
                Estás recibiendo este email porque solicitaste restablecer tu contraseña en KINETIXFITT. Este es un mensaje transaccional y no requiere suscripción.
              </p>
            </td>
          </tr>
        </table>
...
export function buildPasswordResetText(opts: { resetUrl: string; email: string }): string {
  return `Restablece tu contraseña — KINETIXFITT
...
export async function sendPasswordResetEmail(
  ...
  const result = await sendEmail({
    to: email,
    subject: "Restablece tu contraseña — KINETIXFITT",
    html,
    text,
  });
