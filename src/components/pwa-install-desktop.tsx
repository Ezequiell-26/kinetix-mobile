      ...
      <Card className="border-[#D6FF2A]/20 bg-gradient-to-br from-[#D6FF2A]/5 via-zinc-900 to-zinc-900">
        <CardContent className="pt-4 space-y-3">
          <div className="flex gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#D6FF2A] flex items-center justify-center text-black"><Smartphone size={18}/></div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3 space-y-2">
            ...
            <div className="flex gap-2 items-center text-xs text-emerald-400"><Check size={14}/> ¡Listo! Ícono EZEQUIEL en tu inicio, abre como app nativa</div>
          </div>
  ...
  return (
    <Card className="border-[#D6FF2A]/20">
      <CardContent className="pt-4 space-y-3">
        ...
        {deferred ? (
          <Button variant="accent" className="w-full h-11 font-black" onClick={async()=>{ deferred.prompt(); const {outcome}=await deferred.userChoice; if(outcome==="accepted") setDeferred(null); }}><Download size={16} className="mr-2"/> Instalar ahora — 1 click</Button>
        ) : (
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3 text-xs space-y-1">
            ...
            <p className="text-zinc-500">Electron: <span className="font-mono text-xs bg-zinc-800 px-1.5 py-0.5 rounded">cd electron && npm run build:win</span> → .exe</p>
          </div>
        )}
