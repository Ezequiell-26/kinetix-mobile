...
export function TimersHub(){
  ...
  useEffect(()=>{
    ...
    if(sec>0){
      ...
      // Narrador según el bloque que viene; beep solo para presets sin voz (Pomodoro).
      const next = flat[currentExIdx + 1];
      if (currentExIdx < flat.length - 1 && next) {
        if (next.name.includes("Descanso") || next.name.includes("Break")) playTrack("hiitDescansa");
        else if (next.name.includes("Trabajo")) playTrack("hiitTrabaja");
        else { try{ const ctx=new (window.AudioContext|| (window as unknown as {webkitAudioContext: typeof AudioContext}).webkitAudioContext)(); const o=ctx.createOscillator(); o.frequency.value=880; o.connect(ctx.destination); o.start(); setTimeout(()=>o.stop(),150);}catch{} }
  ...
  return (
    <Card className="border-zinc-800">
      <CardHeader><CardTitle className="flex items-center gap-2"><Timer size={18} className="text-primary"/> Cronómetros PRO</CardTitle>
        <div className="flex gap-1.5 overflow-x-auto pb-1">
          {Object.keys(PRESETS).map(k=>(
            <button key={k} onClick={()=>setPreset(k as keyof typeof PRESETS)} className={`px-3 py-1.5 rounded-full text-xs font-bold border whitespace-nowrap ${preset===k?"bg-primary text-black border-primary":"bg-zinc-900 text-zinc-400 border-zinc-800"}`}>{PRESETS[k as keyof typeof PRESETS].title}</button>
          ))}
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="bg-gradient-to-br from-zinc-900 to-zinc-950 border border-zinc-800 rounded-2xl p-6 text-center">
          ...
          <div className="h-1.5 bg-zinc-800 rounded-full overflow-hidden mt-3"><div className="h-full bg-primary" style={{width:`${progress}%`}} /></div>
