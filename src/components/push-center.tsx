    ...
    <Card className={enabled ? "border-emerald-500/20 bg-emerald-500/5" : "border-zinc-800"}>
      ...
      <CardContent className="space-y-3">
        <div className="grid grid-cols-1 gap-2 text-xs">
          <label className="flex items-center justify-between bg-zinc-900 border border-zinc-800 rounded-xl p-3"><span>🏋️ Entreno de hoy — 08:00</span><input type="checkbox" defaultChecked={enabled} className="accent-[#D6FF2A]" /></label>
          <label className="flex items-center justify-between bg-zinc-900 border border-zinc-800 rounded-xl p-3"><span>📋 Check-in pendiente — Domingo 20:00</span><input type="checkbox" defaultChecked={enabled} className="accent-[#D6FF2A]" /></label>
          <label className="flex items-center justify-between bg-zinc-900 border border-zinc-800 rounded-xl p-3"><span>💬 Nuevo mensaje de Ezequiel</span><input type="checkbox" defaultChecked={enabled} className="accent-[#D6FF2A]" /></label>
        </div>
        {perm!=="granted" ? (
        ...
        <p className="text-[11px] text-zinc-600 text-center">OneSignal + FCM + Granite push — Service Worker listo, solo falta VAPID_KEY en .env</p>
      </CardContent>
    </Card>
  );
}
