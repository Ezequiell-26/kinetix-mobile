...
export function OnboardingVideo(){
  return (
    <Card className="border-[#D6FF2A]/20 bg-gradient-to-br from-[#D6FF2A]/5 via-zinc-900 to-zinc-900">
      <CardHeader><CardTitle className="flex items-center gap-2"><Play size={16} className="text-[#D6FF2A]"/> Bienvenida de Ezequiel <Badge variant="accent">Onboarding</Badge></CardTitle><p className="text-xs text-zinc-500">Video 45s + test de movimiento inicial</p></CardHeader>
      <CardContent className="space-y-3">
        <div className="aspect-video rounded-xl bg-zinc-900 border border-zinc-800 flex flex-col items-center justify-center gap-2">
          <div className="w-12 h-12 rounded-full bg-[#D6FF2A] flex items-center justify-center text-black"><Play size={20} className="ml-0.5"/></div>
        ...
        <div className="space-y-2">
          ...
          {["Sentadilla profunda sin dolor","Flexión completa","Plancha 30s","5 dominadas / 10 flexiones"].map(t=>(
            <label key={t} className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 rounded-xl p-2 text-xs"><input type="checkbox" className="accent-[#D6FF2A]"/> {t}</label>
          ))}
        </div>
