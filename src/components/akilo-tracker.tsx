...
import { Scale, Droplets, Ruler, Activity } from "lucide-react";
...
export function AkiloTracker({ measurements }:{ measurements?: Array<{date:string; weight:number|null; bodyFat:number|null; waist:number|null}> }){
  ...
  return (
    <Card className="border-sky-500/20">
      <CardHeader><CardTitle className="flex items-center gap-2"><Scale size={16} className="text-sky-400"/> Akilo — Peso + Medidas <Badge variant="muted">Akilo + LibreFit MIT</Badge></CardTitle><p className="text-xs text-zinc-500">Analytics premium: peso, grasa, cintura + registro rápido</p></CardHeader>
      <CardContent className="space-y-4">
        <div className="h-[140px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data}>
              ...
              <Line type="monotone" dataKey="weight" stroke="#D6FF2A" strokeWidth={2} dot={{r:3}} name="Peso" />
        ...
        <Button variant="accent" className="w-full" onClick={()=>console.log("akilo save", {weight, bodyFat, waist})}>Guardar medición</Button>
        <p className="text-[11px] text-zinc-600 text-center">Akilo + LibreFit MIT — peso, grasa, medidas + tracking biblioteca</p>
      </CardContent>
    </Card>
  );
}
