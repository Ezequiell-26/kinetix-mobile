...
import { Trophy, Flame, Target, Calendar, Dumbbell, Zap, Award, Star } from "lucide-react";
...
export function Achievements({ data }:{ data: AchData }){
  ...
  return (
    <Card className="border-[#D6FF2A]/20 bg-gradient-to-br from-[#D6FF2A]/5 via-zinc-900 to-zinc-900">
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><Trophy size={18} className="text-[#D6FF2A]"/> Logros <Badge variant="accent">Strive MIT</Badge> <span className="ml-auto text-xs font-mono bg-zinc-800 px-2 py-1 rounded-full">Nv {level} • {totalXp} XP</span></CardTitle>
        <div className="h-2 bg-zinc-800 rounded-full overflow-hidden mt-2"><div className="h-full bg-[#D6FF2A] transition-all" style={{width:`${levelProgress}%`}} /></div>
      ...
      <CardContent className="grid sm:grid-cols-2 gap-3">
        {achievements.map(a=>{
          const Icon=a.icon;
          return (
            <div key={a.id} className={`p-3 rounded-xl border flex gap-3 items-center ${a.unlocked?"bg-zinc-800 border-[#D6FF2A]/30":"bg-zinc-900 border-zinc-800 opacity-60"}`}>
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${a.unlocked?"bg-[#D6FF2A] text-black":"bg-zinc-800 text-zinc-500"}`}><Icon size={18}/></div>
              <div className="flex-1 min-w-0">
                <p className="font-bold text-sm flex items-center gap-1">{a.name} {a.unlocked && <span className="text-emerald-400 text-xs">✓</span>} <span className="text-[10px] bg-zinc-700 px-1.5 py-0.5 rounded-full ml-auto">+{a.xp} XP</span></p>
                <p className="text-xs text-zinc-500">{a.desc}</p>
                <div className="h-1 bg-zinc-700 rounded-full overflow-hidden mt-1"><div className={`h-full ${a.unlocked?"bg-[#D6FF2A]":"bg-zinc-600"}`} style={{width:`${a.progress}%`}} /></div>
              </div>
            </div>
          );
        })}
