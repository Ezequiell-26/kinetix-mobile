...
const MODES = {
  ...
  hiit: {label:"HIIT", work:30, rest:15, rounds:8, color:"bg-[#D6FF2A]"},
};
...
export function OpenHiitPro(){
  ...
  useEffect(()=>{ if(!running) return; if(sec>0){ const t=setTimeout(()=>setSec(s=>s-1),1000); return ()=>clearTimeout(t);} else { try{ const ctx=new (window.AudioContext|| (window as unknown as {webkitAudioContext: typeof AudioContext}).webkitAudioContext)(); const o=ctx.createOscillator(); o.frequency.value=isWork?880:440; o.connect(ctx.destination); o.start(); setTimeout(()=>o.stop(),200);}catch{} if(isWork && rest>0){ setIsWork(false); setSec(rest);} else if(!isWork){ if(current < rounds){ setCurrent(c=>c+1); setIsWork(true); setSec(work);} else { setRunning(false); setCurrent(1); setIsWork(true); setSec(work);} } else { if(current < rounds){ setCurrent(c=>c+1); setSec(work);} else { setRunning(false); setCurrent(1); setSec(work);} } } },[running, sec, isWork, work, rest, rounds, current]);

  const cfg=MODES[mode];
  return (
    <Card className="border-zinc-800">
      <CardHeader><CardTitle className="flex items-center gap-2"><Timer size={18} className="text-[#D6FF2A]"/> OpenHIIT Pro <Badge variant="accent">Unbroken+Simple MIT</Badge></CardTitle><p className="text-xs text-zinc-500">Tabata • EMOM • AMRAP • HIIT — audio cues + Tactical Barbell</p></CardHeader>
      <CardContent className="space-y-4">
        <div className="flex gap-2 overflow-x-auto pb-1">
          {(Object.keys(MODES) as Array<keyof typeof MODES>).map(k=>(
            <button key={k} onClick={()=>setMode(k)} className={`px-3 py-1.5 rounded-full text-xs font-bold border whitespace-nowrap ${mode===k?"bg-[#D6FF2A] text-black border-[#D6FF2A]":"bg-zinc-900 text-zinc-400 border-zinc-800"}`}>{MODES[k].label}</button>
          ))}
        </div>
        ...
        <p className="text-[11px] text-zinc-600 text-center">OpenHIIT + Unbroken + Simple MIT — intervalos premium offline</p>
      </CardContent>
    </Card>
  );
}
