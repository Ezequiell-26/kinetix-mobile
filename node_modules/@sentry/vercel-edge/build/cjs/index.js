...
function setContextOnScope(scope, context2) {
  core.addNonEnumerableProperty(scope, SCOPE_CONTEXT_FIELD, core.makeWeakRef(context2));
}
...
class SentryPropagator extends W3CBaggagePropagator {
  ...
  inject(context2, carrier, setter) {
    ...
    if (!core.shouldPropagateTraceForUrl(url, tracePropagationTargets, this._urlMatchesTargetsMap)) {
      DEBUG_BUILD$1 && core.debug.log("[Tracing] Not injecting trace data for url because it does not match tracePropagationTargets:", url);
      return;
    }
...
function buildContextWithSentryScopes(context2, activeContext) {
  ...
  if (span?.spanContext().traceState?.get(SENTRY_TRACE_STATE_CHILD_IGNORED) === "1") {
    const contextWithoutSpan = api.trace.deleteSpan(context2);
...
function getParentSampled(parentSpan, traceId, spanName) {
  ...
  if (api.isSpanContextValid(parentContext) && parentContext.traceId === traceId) {
    if (parentContext.isRemote) {
      ...
      DEBUG_BUILD$1 && core.debug.log(`[Tracing] Inheriting remote parent's sampled decision for ${spanName}: ${parentSampled2}`);
...
function getStoreWithActiveSpan(span) {
  ...
  const isIgnoredChild = core.spanIsIgnored(span) && core.getRootSpan(span) !== span || span.spanContext().traceState?.get(SENTRY_TRACE_STATE_CHILD_IGNORED) === "1";
