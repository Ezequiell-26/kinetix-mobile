...
import { GLOBAL_OBJ, debug, applySdkMetadata, addVercelAiProcessors, getGlobalScope, getClient, SEMANTIC_ATTRIBUTE_SENTRY_OP } from '@sentry/core';
import { experimentalUseDiagnosticsChannelInjection as experimentalUseDiagnosticsChannelInjection$1, getDefaultIntegrations, httpIntegration, isDiagnosticsChannelInjectionEnabled, init as init$1 } from '@sentry/node';
...
function experimentalUseDiagnosticsChannelInjection() {
  experimentalUseDiagnosticsChannelInjection$1();
}
...
function init(options) {
  ...
  const customDefaultIntegrations = getDefaultIntegrations(options).filter((integration) => integration.name !== "Http").concat(
  ...
  client?.on("beforeSampling", ({ spanAttributes }, samplingDecision) => {
    if (
      ...
      typeof spanAttributes[HTTP_TARGET] === "string" && // eslint-disable-next-line typescript/no-deprecated
      spanAttributes[HTTP_TARGET].includes("sentry_key") && // eslint-disable-next-line typescript/no-deprecated
      spanAttributes[HTTP_TARGET].includes("sentry_client") || typeof spanAttributes[URL_QUERY] === "string" && spanAttributes[URL_QUERY].includes("sentry_key") && spanAttributes[URL_QUERY].includes("sentry_client")
    ) {
...
export { ErrorBoundary, createReduxEnhancer, experimentalUseDiagnosticsChannelInjection, init, showReportDialog, withErrorBoundary };
