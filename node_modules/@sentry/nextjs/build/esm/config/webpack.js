...
import { externalizeOrchestrionRuntimePackages } from './diagnosticsChannelInjection.js';
import { normalizePathForGlob, getBuildPluginOptions } from './getBuildPluginOptions.js';
import { loadOrchestrionBundler } from './loadOrchestrionBundler.js';
  ...
  return function newWebpackFunction(incomingConfig, buildContext) {
    ...
    if (!(isDev || isStaticExport && isServer)) {
      ...
      if (sentryWebpackPlugin) {
        ...
        const projectDir2 = normalizePathForGlob(dir);
        const distDir = normalizePathForGlob(userNextConfig2.distDir ?? ".next");
    ...
    if (runtime === "server" && userSentryOptions._experimental?.useDiagnosticsChannelInjection) {
      newConfig.plugins.push(
        loadOrchestrionBundler().sentryOrchestrionWebpackPlugin()
      );
